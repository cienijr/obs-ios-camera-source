...
static void init_hw_decoder(struct ffmpeg_decode *d)
{
    ...
    if (hw_ctx) {
        const char *hw_type_name = av_hwdevice_get_type_name(*priority);
        blog(LOG_INFO, "using hwdevice %s", hw_type_name);
...
int ffmpeg_decode_init(struct ffmpeg_decode *decode, enum AVCodecID id, bool hw)
{
    int ret;
    ...
    // skip random stuff (mostly SEIs generated by VT)
    decode->decoder->skip_frame = AVDISCARD_NONREF;
    ...
    if (!hw) {
        // this breaks when using hardware acceleration
...
void ffmpeg_decode_free(struct ffmpeg_decode *decode)
{
    ...
    if (decode->hw_frame) {
        av_frame_unref(decode->hw_frame);
        av_free(decode->hw_frame);
    }

    if (decode->hw_ctx) {
        av_buffer_unref(&decode->hw_ctx);
    }
                         ...
                         bool *got_output)
{
    ...
    *got_output = false;
    ...
    AVFrame *receiveFrame = decode->frame;

    if (decode->hw) {
        // hardware acceleration is enabled and is available
        if (!decode->hw_frame)
        {
        ...
        receiveFrame = decode->hw_frame;
    }

    ret = avcodec_send_packet(decode->decoder, &packet);
    if (ret == 0)
        ret = avcodec_receive_frame(decode->decoder, receiveFrame);
    ...
    AVFrame *targetFrame = receiveFrame;
    if (decode->hw) {
        if (decode->hw_frame->format == decode->hw_format) {
            ret = av_hwframe_transfer_data(decode->frame, decode->hw_frame, 0);
            ...
            targetFrame = decode->frame;
        }
    }

    for (size_t i = 0; i < MAX_AV_PLANES; i++)
    {
        frame->data[i] = targetFrame->data[i];
        frame->linesize[i] = targetFrame->linesize[i];
    }

    new_format = convert_pixel_format(targetFrame->format);
